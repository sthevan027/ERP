...
- **Configurar Supabase corretamente no ambiente**
  - **Ação**: garantir `web/.env.local` com `VITE_SUPABASE_URL` e `VITE_SUPABASE_ANON_KEY`.
  - **Ação**: reiniciar `pnpm dev` após qualquer alteração de `.env`.
  - **Aceite**: `/setup` só aparece quando faltar env; com env ok, vai para `/login`.
...
- **Bootstrap mínimo do banco (dados iniciais)**
  - **Ação**: criar `orgs`, `warehouses`, `clients` (manual ou seed).
  - **Aceite**: gestor consegue entrar e ver dashboard sem telas “vazias”.
...
- **Padronizar “base URL / headers / timeout” nas chamadas do Supabase**
  - **Ação**: criar um wrapper de requests/erros em `web/src/lib/`:
    - timeout único (ex: 12s) + retry leve para GET (ex: 1 retry)
    - mensagens amigáveis para `Failed to fetch`, `401`, `403`, `JWT expired`
...
- **Revisar estados de loading em todas as páginas**
  - **Ação**: criar componentes `LoadingCard` e `ErrorCard`.
